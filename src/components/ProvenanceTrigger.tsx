...
import { dispatchProvenance } from './ProvenanceDrawer';
...
export function ProvenanceTrigger({ valueId, label = 'De onde vem?', compact = false }: ProvenanceTriggerProps) {
  return (
    <button
      type="button"
      onClick={() => dispatchProvenance({ valueId })}
      className={
        compact
