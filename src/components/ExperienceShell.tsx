...
export function ExperienceShell({ children }: { readonly children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    return () => { window.removeEventListener('scroll', onScroll); window.removeEventListener('observatorio:command', onOpen); window.removeEventListener('observatorio:election-mode', onElection as EventListener); };
  }, []);
