...
const RECENT_KEY = 'observatorio-recent-sections-v32', FAVORITES_KEY = 'observatorio-favorite-sections-v32', REDUCED_KEY = 'observatorio-reduced-motion-v32', FAST_MODE_KEY = 'observatorio-fast-reading-v33-default-investigation';
...
export function ExperienceShell({ children }: { readonly children: ReactNode }) {
  ...
  useEffect(() => {
    setRecent(readList(RECENT_KEY)); setFavorites(readList(FAVORITES_KEY)); const reduced = localStorage.getItem(REDUCED_KEY) === '1'; setReducedMotion(reduced); document.documentElement.classList.toggle('reduced-motion', reduced); const fast = localStorage.getItem(FAST_MODE_KEY) === '1'; setFastReading(fast); document.documentElement.classList.toggle('fast-reading', fast);
