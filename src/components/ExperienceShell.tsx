...
const RECENT_KEY = 'observatorio-recent-sections', FAVORITES_KEY = 'observatorio-favorite-sections', REDUCED_KEY = 'observatorio-reduced-motion', FAST_MODE_KEY = 'observatorio-fast-reading-v28';
...
export function ExperienceShell({ children }: { readonly children: ReactNode }) {
  const [commandOpen, setCommandOpen] = useState(false), [helpOpen, setHelpOpen] = useState(false), [query, setQuery] = useState(''), [progress, setProgress] = useState(0), [recent, setRecent] = useState<string[]>([]), [favorites, setFavorites] = useState<string[]>([]), [reducedMotion, setReducedMotion] = useState(false), [fastReading, setFastReading] = useState(false);
  ...
  useEffect(() => {
    setRecent(readList(RECENT_KEY)); setFavorites(readList(FAVORITES_KEY)); const reduced = localStorage.getItem(REDUCED_KEY) === '1'; setReducedMotion(reduced); document.documentElement.classList.toggle('reduced-motion', reduced); const fast = localStorage.getItem(FAST_MODE_KEY) !== '0'; setFastReading(fast); document.documentElement.classList.toggle('fast-reading', fast);
  ...
  const toggleFastReading = () => { const next = !fastReading; setFastReading(next); localStorage.setItem(FAST_MODE_KEY, next ? '1' : '0'); document.documentElement.classList.toggle('fast-reading', next); };
  ...
  return <>
    ...
    {children}
    <div className="quick-dock" aria-label="Acesso rápido"><button type="button" onClick={() => { jump('dashboard'); remember('dashboard'); }} title="Dashboard" aria-label="Ir para Dashboard">D</button><button type="button" onClick={() => { jump('transporte'); remember('transporte'); }} title="Transporte" aria-label="Ir para Transporte">T</button><button type="button" onClick={() => { jump('saude'); remember('saude'); }} title="Saneamento e saúde" aria-label="Ir para saneamento e saúde">S</button><button type="button" onClick={() => { jump('politica'); remember('politica'); }} title="Pesquisas e integridade" aria-label="Ir para pesquisas e integridade">P</button><button type="button" onClick={toggleFastReading} title={fastReading ? "Modo investigação" : "Modo leitura rápida"} aria-label={fastReading ? "Ativar modo investigação" : "Ativar modo leitura rápida"}>{fastReading ? "↕" : "≡"}</button></div>
