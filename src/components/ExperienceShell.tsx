...
export function ExperienceShell({ children }: { readonly children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!isCompactViewport) {
      window.setTimeout(() => (commandOpen ? inputRef.current : modalRef.current?.querySelector<HTMLElement>('button,[href],[tabindex]:not([tabindex="-1"])'))?.focus(), 0);
    }
