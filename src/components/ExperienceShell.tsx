...
export function ExperienceShell({ children }: { readonly children: ReactNode }) {
  ...
  useEffect(() => {
    setRecent(readList(RECENT_KEY)); setFavorites(readList(FAVORITES_KEY)); const reduced = localStorage.getItem(REDUCED_KEY) === '1'; setReducedMotion(reduced); document.documentElement.classList.toggle('reduced-motion', reduced); const storedMode = localStorage.getItem(MODE_KEY) as ExperienceMode | null; const legacyFast = localStorage.getItem(LEGACY_FAST_MODE_KEY) === '1'; const initialMode: ExperienceMode = storedMode === 'overview' || storedMode === 'investigation' || storedMode === 'evidence' ? storedMode : legacyFast ? 'overview' : 'overview'; setMode(initialMode); document.documentElement.dataset.experienceMode = initialMode; document.documentElement.classList.remove('mode-overview','mode-investigation','mode-evidence'); document.documentElement.classList.add('mode-' + initialMode);
  ...
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      ...
      if (pendingG.current) {
        const map: Record<string, NavigationId> = { d:'dashboard',c:'contexto',e:'eleitorado',t:'transporte',p:'politica',x:'eleitoral360',o:'orcamento',a:'dados',q:'qualidade',f:'fontes',v:'evidencias',u:'acao',l:'linha-do-tempo' };
  ...
  return <>
    ...
    {children}
    <div className="quick-dock" aria-label="Acesso rápido"><button type="button" onClick={() => { jump('dashboard'); remember('dashboard'); }} title="Dashboard" aria-label="Ir para Dashboard"><LayoutDashboard className="h-4 w-4" aria-hidden="true" /></button><button type="button" onClick={() => { jump('transporte'); remember('transporte'); }} title="Transporte" aria-label="Ir para Transporte"><BusFront className="h-4 w-4" aria-hidden="true" /></button><button type="button" onClick={() => { jump('saude'); remember('saude'); }} title="Saneamento e saúde" aria-label="Ir para saneamento e saúde"><Droplets className="h-4 w-4" aria-hidden="true" /></button><button type="button" onClick={() => { jump('politica'); remember('politica'); }} title="Pesquisas e integridade" aria-label="Ir para pesquisas e integridade"><Search className="h-4 w-4" aria-hidden="true" /></button><button type="button" onClick={() => setExperienceMode(mode === 'overview' ? 'investigation' : mode === 'investigation' ? 'evidence' : 'overview')} className="bg-sky-300/10 text-sky-200" title={'Modo atual: ' + MODE_LABELS[mode] + '. Clique para alternar.'} aria-label={'Modo atual: ' + MODE_LABELS[mode] + '. Alternar modo.'}>⇄</button></div>
