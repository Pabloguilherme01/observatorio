        ...
        <div className="grid gap-3 md:grid-cols-3">
          {rows.map(([label, amount]) => (
            <div key={label} className="rounded-2xl border border-white/8 p-4">
              ...
              <div className="mt-1 text-xs text-slate-500">{population > 0 ? `${formatBRL(amount / population)} por habitante, usando população estimada de 2026` : 'População de referência indisponível para cálculo per capita'}</div>
            </div>
          ))}
        </div>
