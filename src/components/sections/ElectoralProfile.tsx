...
export function ElectoralProfile() {
  ...
  return (
    <section id="eleitorado" className="mx-auto max-w-7xl px-4 py-14 sm:px-6" aria-labelledby="eleitorado-title">
      ...
      <div className="grid gap-4 lg:grid-cols-[1.15fr_.85fr]">
        <Card>
          <div className="grid gap-5 lg:grid-cols-[auto_1fr] lg:items-center">
            ...
            <div>
              <div className="text-xs font-bold uppercase tracking-[0.15em] text-slate-500">Visão principal</div>
              <div className="mt-3 grid gap-3 sm:grid-cols-3">
                <div className="rounded-2xl border border-white/8 p-4 light:border-slate-200">
                  <UserRound className="mb-2 h-4 w-4 text-sky-300" aria-hidden="true" />
                  <strong className="block text-white">{(d.electoral.womenPct ?? 0).toFixed(2).replace('.', ',')}%</strong>
                  <span className="text-xs text-slate-500">mulheres</span>
                </div>
                <div className="rounded-2xl border border-white/8 p-4 light:border-slate-200">
                  <UsersRound className="mb-2 h-4 w-4 text-sky-300" aria-hidden="true" />
                  <strong className="block text-white">{(d.electoral.menPct ?? 0).toFixed(2).replace('.', ',')}%</strong>
                  <span className="text-xs text-slate-500">homens</span>
                </div>
                <div className="rounded-2xl border border-white/8 p-4 light:border-slate-200">
                  <Fingerprint className="mb-2 h-4 w-4 text-sky-300" aria-hidden="true" />
                  <strong className="block text-white">{formatNumber(d.electoral.socialNameCount ?? 0)}</strong>
      ...
      <Card className="mt-4 border-sky-300/12 bg-sky-300/[0.025]">
        ...
        <div className="mt-3 grid gap-4 lg:grid-cols-[auto_1fr] lg:items-center">
          ...
          <div className="text-sm leading-6 text-slate-400">
            ...
            <a className="ml-1 inline-flex items-center gap-1 font-bold text-sky-300 hover:text-sky-200" href="https://www12.senado.leg.br/tv/programas/noticias-1/2024/10/aguas-lindas-e-o-municipio-do-entorno-do-df-que-mais-recebeu-titulos-transferidos-da-capital-federal" target="_blank" rel="noopener noreferrer">Conferir fonte</a>
          </div>
        </div>
      </Card>
    </section>
