    ...
    <section id="insights" className="mx-auto max-w-7xl px-4 py-14 sm:px-6" aria-labelledby="insights-title">
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        ...
        <Card id="healgo">
          ...
          <div className="mt-4 grid grid-cols-3 gap-3 text-center">{[['Atend./leito', Math.round(pressure.perBed).toLocaleString('pt-BR')],['Redução', Math.max(0, pressure.reduction).toFixed(1).replace('.', ',') + '%'],['Leitura', pressure.status]].map(([label,value]) => <button key={label} type="button" onClick={() => dispatchInspect({ label: 'HEAL · ' + label, value, sourceId: 'healgo', status: 'derivado', note: 'Resultado de simulação matemática; a classificação de pressão é heurística do observatório.' })} className="rounded-2xl bg-white/[0.02] p-3 text-left"><span className="block text-xs text-slate-500">{label}</span><strong className="mt-1 block text-lg text-white">{value}</strong></button>)}</div>
