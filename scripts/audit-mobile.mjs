...
must(files.mobileNav.includes('if (!moreOpen) return;') && files.mobileNav.includes("querySelector<HTMLButtonElement>('[role=\"menuitem\"]')?.focus()") && files.mobileNav.includes('window.cancelAnimationFrame(focusFrame)'), 'menu Mais move foco somente após o portal renderizar e limpa RAF pendente');
