...
must(files.mobileNav.includes('restoreFocusRef') && files.mobileNav.includes("querySelector<HTMLButtonElement>('[role=\"menuitem\"]')?.focus()") && files.mobileNav.includes('moreButtonRef.current?.focus()') && files.mobileNav.includes('window.cancelAnimationFrame(frame)'), 'menu Mais move foco após renderização e restaura o acionador de forma determinística');
