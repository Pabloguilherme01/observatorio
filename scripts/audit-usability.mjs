...
for (const link of externalLinks) {
  ...
  if (/target=["']_blank["']/.test(link.context) && !/rel=["'][^"']*noopener/.test(link.context)) {
    fail('Link target=_blank sem rel=noopener: ' + link.url);
  }
}
...
for (const [id, label] of [
  ...
  ['mudancas-snapshot', 'histórico de mudanças'],
...
if (ambiguous.length > 8) warn(ambiguous.length + ' rótulos de botão potencialmente genéricos; prefira verbos com destino explícito');
